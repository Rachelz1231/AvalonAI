...
config_list = [
    {
        "model": "gpt-4",
        "api_key": "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",
    }
]
...
def initialize_game():
    ...
    for name, role in players_with_roles.items():
        if name != "User":
            ...
            log(f"[USER ROLE INFO] Your role is: {user_role}. {role_descriptions[user_role]}")
    ...
    if merlin.name != user_player.name:
        ...
        log(f"As Merlin, you know the evil agents are: {assassin.name} and {morgana.name}")
    ...
    if percival.name != user_player.name:
        ...
        log(f"As Percival, you see both {merlin.name} and {morgana.name} as potential Merlins and cannot distinguish between them.")
    ...
    if user_player.role in ["Assassin", "Morgana"]:
        ...
        log(f"As an evil agent, your evil teammate is: {', '.join(evil_teammates)}")
        for teammate in evil_teammates:
            user_player.add_teammate(teammate)

    # 3. log hidden mapping (including user)
...
def run_quest(selected_agents, conversation_history):
    log("\n=== QUEST BEGINS ===\n")
    log(f"Selected players for this quest: {', '.join([a if isinstance(a, str) else a.name for a in selected_agents])}")
        ...
        if agent == "User":
            ...
            while True:
                ...
                if user_vote in ["SUCCEED", "FAIL"]:
                    break
                log("Invalid input. Please type 'SUCCEED' or 'FAIL'.")
            vote = user_vote
        else:
...
def select_agents_for_quest(is_initial=False):
    if is_initial:
        ...
        log("Select three players for the initial team (e.g., 'A B C' or 'AgentA AgentB AgentC' or 'User'):")
    else:
        log("\n=== FINAL TEAM SELECTION ===\n")
        log("Select three players for the quest (e.g., 'A B C' or 'AgentA AgentB AgentC' or 'User'):")
    ...
    while True:
        ...
        if len(selected_agents) != 3:
            log(f"Please select exactly 3 players. You selected {len(selected_agents)}.")
            continue

        # Confirm selection
        log(f"You've selected: {', '.join([a if isinstance(a, str) else a.name for a in selected_agents])}")
